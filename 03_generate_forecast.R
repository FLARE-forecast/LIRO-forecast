...
forecast_start_datetime_UTC <- lubridate::with_tz(forecast_start_datetime_local, tzone = "CST6CDT")
...
if(length(forecast_files) > 0){
  ...
  observed_met_file <- file.path(config$qaqc_data_location,"observed-met_liro.nc")
  ...
  obs <- flare::create_obs_matrix(cleaned_observations_file_long,
                                  ...
                                  end_datetime_local,
                                  local_tzone = config$local_tzone,
                                  modeled_depths = as.numeric(config$modeled_depths))
